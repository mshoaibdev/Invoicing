...
const { updateRole,respResult, isLoading, errors, getRole, role } = useRoles()
...
onMounted(async () => {
  if(props.isDialogVisible){
    ...
    // set form data with permission ids only
    formData.value = {
      ...role.value,
...
const onSubmit = async() => {
  refForm.value?.validate().then(async ({ valid: isValid }) => {
    if (isValid){
      ...
      // append put method to form data
      formData.value._method = 'PUT'
      
      await updateRole(formData.value, props.roleId)
